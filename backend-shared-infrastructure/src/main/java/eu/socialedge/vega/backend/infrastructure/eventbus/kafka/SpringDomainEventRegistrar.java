...
import eu.socialedge.vega.backend.ddd.DomainEventSubscriber;
import lombok.val;
...
public class SpringDomainEventRegistrar implements DomainEventRegistrar {
    ...
    private final Map<Class<?>, DomainEventSubscriber<?>> subscribers = new HashMap<>();

    @Override
    public <T extends DomainEvent> void registerEventSubscriber(DomainEventSubscriber<T> eventSubscriber, Class<T> eventType) {
        subscribers.put(eventType, eventSubscriber);
    }

    @Override
    public void deregisterEventSubscriber(Class<?> eventType) {
    ...
    <T extends DomainEvent> void handleEvent(T event) {
        ...
        try {
            if (domainEventSubscriber != null)
                ((DomainEventSubscriber<T>) domainEventSubscriber).handleEvent(event);
        } catch (Exception e) {
