...
public class Path2dConverter implements AttributeConverter<Path2D, String> {
    ...
    @Override
    public Path2D convertToEntityAttribute(String rawPath2d) {
        if (isBlank(rawPath2d))
            return null;

        try {
            List<Location> vertices = objectMapper.readValue(rawPath2d, new TypeReference<List<Location>>() {});
