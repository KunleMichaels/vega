...
public class DeductionSerializer implements AttributeConverter<Deduction, String> {
    ...
    @Override
    public String convertToDatabaseColumn(Deduction deduction) {
        return Optional.ofNullable(deduction)
            .map(d -> d.getClass().getName())
            .orElse(null);
    }

    @Override
    public Deduction convertToEntityAttribute(String rawDeduction) {
        if (isBlank(rawDeduction))
            return null;

        try {
            return (Deduction) Class.forName(rawDeduction).newInstance();
        } catch (Exception e) {
           throw new SerializationException("Failed to deserialize Deduction strategy " +
                   "from str = " + rawDeduction, e);
        }
    }
}
